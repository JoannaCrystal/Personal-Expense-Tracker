...
def create_expense(db: Session, expense: ExpenseCreate, account_id: int) -> Expense:
    db_expense = Expense(
        ...
        category_id=expense.category_id  # optional, can be None
    )
    db.add(db_expense)
    db.commit()
...
def get_expenses_by_filters(db: Session, account_id: int, start_date: Optional[str] = None, end_date: Optional[str] = None, category_id: Optional[int] = None) -> List[Expense]:
